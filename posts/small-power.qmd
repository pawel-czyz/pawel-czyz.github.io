...
For example, if we want coverage $\alpha = 95\%$, we take $\alpha=5\%$
...
Both comparisons are equivalent and equally hard to compute: the hard of comparing $|T_0|$ against $u$ is calculation of $u$ from $\alpha$ (which requires the access to the CDF).
...
(As a side note, I already regret not implementing $p$-value calculation in JAX.)
