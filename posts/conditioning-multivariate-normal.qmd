...
For $\mathcal X = \{1, \dotsc, n\}$ we considered am indexed collection of random variables $\{Y_x\}_{x\in \mathcal X}$. Let's call it a *stochastic process*. 

This stochastic process had the property that the joint distribution was multivariate normal.
From that we could use the fact that all the other distributions $P(Y_{x_1}, \dotsc, Y_{x_m})$ were again multivariate normal, what allowed us to do prediction via conditioning (which resulted, again, in multivariate normal distributions).
