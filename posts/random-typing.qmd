...
Interestingly, the teacher could look at the results and tell some students that they did not toss the coin, but rather simply wrote down the results just before the lesson started!
...
ax.hist(sequence_lengths, bins=10)
...
It simply counts the number of $1$ characters within each sequence and, treating them as independent and identically distributed, estimates the probability of typing $1$.
...
for seq in sequences:
  ...
  p = np.mean(s)
  ps.append(p)
  wald_int = wald_interval(n, x)
  wils_int = wilson_interval(n, x)
  intervals_wald.append(wald_int)
  intervals_wils.append(wils_int)
...
for ax, ints, name in zip(axs.ravel(), [intervals_wald, intervals_wils], ["Wald", "Wilson"]):
  xs = np.arange(1, len(ps) + 1)
  ps_ = np.asarray(ps)
  ints_ = np.array(ints).T
  ints_ = np.abs(ints_ - ps_[None, :])
  ax.errorbar(xs, ps, yerr=ints_)
  ax.set_title(f"{name} confidence intervals")
  ax.set_xlabel("Sequence number")
  ax.set_ylabel("Probability estimate")
  ax.axhline(0.5, linestyle="--", color="white")

fig.tight_layout()
```
import jax
If we look at the individual sequences, it may seem that I sometimes depart from the probability of 50%, however the confidence intervals are still quite wide and include the 50% value.
Drawing the point estimates (which is itself dubious practice, as they are quite noisy) also shows that they are around 50%:
```{python}
fig, ax = plt.subplots()
ax.hist(ps, bins=10)
ax.set_xlabel("Estimate $\\hat p$")
ax.axvline(0.5, linestyle="--", color="white")
```

A better way would be to build a hierarchical model.
Say, this one, where we draw the probabilities from a beta distribution with unknown parameters:
```{python}
import jax
...
def bag_of_coins(ns, xs):
    # weakly-informative log-normal hyperpriors
    ...
    beta  = numpyro.deterministic("beta",  jnp.exp(beta_raw))

    # BetaBinomial already integrates over p_i
    numpyro.sample(
        "x",
        dist.BetaBinomial(concentration1=alpha,
                          concentration0=beta,
                          total_count=ns),
        obs=xs
    )
...
nuts      = NUTS(bag_of_coins)
num_samples = 1000
mcmc      = MCMC(nuts, num_warmup=num_samples, num_samples=num_samples, progress_bar=True)
mcmc.run(rng_key, ns=total_counts, xs=n_ones)

def f(key, sample):
  return jax.random.beta(key, sample["alpha"], sample["beta"])

predicted_ps = jax.vmap(f)(jax.random.split(rng_key, num_samples), mcmc.get_samples())

fig, ax = plt.subplots()
ax.hist(predicted_ps, density=True, bins=20)
ax.axvline(0.5, linestyle="--", color="white")
ax.set_xlabel("Inferred probabilities")
```

Right. It seems that the frequencies of both 0 and 1 are quite similar.
Let's move to actual autoregressive models, which model the dependencies.
