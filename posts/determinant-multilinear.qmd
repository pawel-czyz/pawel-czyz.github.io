...
The definition above has a lot of advantages, but it also has an important drawback --- the "why" behind this construction is hidden and then appears later in a long list of its properties.
...
However, *not* all theorems and exercises will work with finite fields (especially $\mathbb F_2 = \mathbb Z/2\mathbb Z$). 
...
with the following rules:
  ...
  1. The wedge $\wedge$ operator is bilinear, i.e., $\lambda (v+v')\wedge w = \lambda v\wedge w + v'\wedge w$ and $\lambda v\wedge (w+w') = \lambda v\wedge w + v\wedge w'. 
  2. $\wedge$ is antisymmetric, i.e., $v\wedge w = -w\wedge v$ (or you should recognise that it's equivalent to $v\times v=0$).
  3. If $e_1, \dotsc, e_n$ is a basis of $V$, then
    $$
    e_1\wedge e_2, e_1\wedge e_3, \dotsc, e_1\wedge e_n, e_2\wedge e_3, \dotsc, e_2\wedge e_n, \dotsc, e_{n-1}\wedge e_n
    $$ 
...
$$
\lambda_1 a_1\wedge a_2\cdots\wedge a_k + \cdots + \lambda_n v_1\wedge v_1 \cdots\wedge v_k
$$
...
Let $f\colon V\times V \cdots\times V\to U$ be an antisymmetric multilinear function. Then, there exists a unique linear mapping $\tilde f\colon \bigwedge^k V\to U$ such that

$$ f = \tilde f\circ \mu, $$

where $\mu(v_1, \dotsc, v_k) = v_1\wedge \dotsc \wedge v_k$.
...
For $A\colon V\to V$ we consider the induced mapping $A'\colon \bigwedge^n V\to \bigwedge^n V$ given by the antisymmetric and multilinear mapping
$$
(v_1, \dotsc, v_n) \mapsto (Av_1) \wedge \cdots (Av_n).
$$

Because $\bigwedge^n V$ is one-dimensional, this mapping must be multiplication by a number. Namely, we define the determinant $\det A$ to be the number such that:
$$
Av_1 \wedge \dotsc Av_n = \det A\, (v_1\wedge \dotsc \wedge v_n).
$$
:::
...
I like this geometric intuition, especially that it is clear that determinant depends only on the linear map, rather than a matrix representing it in a chosen basis. 
...
Similarly, it's clear that $\det \left(k\cdot \mathrm{id}_V) = k^{\dim V}$.
:::
...
Establishing such isomorphisms is quite a nice technique, which also can be used to proof
...
$$
If $i\colon V\to V\oplus W$ and $j\colon W\to V\oplus W$, this isomorphism is given as
$$
