...
Typically, adjacent indices are swapped, as for $i$ very distant from $j$ we expect that $r$ is close to zero. 

Let's implement this in JAX!
...
1. The reference distribution should cover a large enough area, so that the modes are covered. Using a reference distribution with a very small standard deviation resulted in non-mixing chain.  
2. Ensuring that local exploration is somewhat efficient is very important. If I used $\sigma$ to sample from $p_0$ (rather than using random walk with an appropriate scale), I could not get a good mixing of the chain.
3. Sometimes the base in the exponential annealing schedule could qualitatively change the results.