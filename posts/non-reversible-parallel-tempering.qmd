...
xs_deo_all = np.empty((len(SIGMAS), len(DISTRIBUTIONS), N_SAMPLES))
...
n_chains = 30
betas = annealing_exponential(n_chains, 1.4)

rejections_deo_all = np.empty((len(SIGMAS), len(DISTRIBUTIONS), n_chains - 1))
...
for d, (_, dist_factory) in enumerate(DISTRIBUTIONS.items()):
  ...
  for s, sigma in enumerate(SIGMAS):
    ...
    key, subkey = jrandom.split(key)
    x0 = 0.5 * jnp.ones(n_chains)
    xs_pt, rejections = deo_sampling_loop(key=subkey, x0=x0, kernel_local=K_ind, kernel_deo=K_deo, n_samples=N_SAMPLES * 2)
    
    xs_pt = xs_pt[N_SAMPLES:]
    rejections = rejections[N_SAMPLES:]
    rejections = rejections.mean(axis=0)

    xs_deo_all[s, d, :] = xs_pt[:, -1]
    rejections_deo_all[s, d, :] = rejections
```
...
for i, sigma in enumerate(SIGMAS):
  ...
  for j, (dist_name, dist_factory) in enumerate(DISTRIBUTIONS.items()):
    ...
    ax.hist(xs_deo_all[i, j], density=True, bins=bins, histtype="stepfilled", color="C3", alpha=0.4)
...
for i, sigma in enumerate(SIGMAS):
  ...
  for j, (dist_name, dist_factory) in enumerate(DISTRIBUTIONS.items()):
    ...
    ax.plot(xs_deo_all[i, j], color="C3")
...
Let's apply these utilities to suggest better annealing schedules for the problems above.
We can visualise the $\Lambda$ curves together with the current schedule:
...
for i, sigma in enumerate(SIGMAS):
  ...
  for j, (dist_name, dist_factory) in enumerate(DISTRIBUTIONS.items()):
    ...
    reject_rates = rejections_deo_all[i, j]

    lambd_fn = get_lambda_function(betas, estimate_lambda_values(reject_rates))
    ...
    for beta in betas:
      ax.axvline(beta, c="grey", linestyle="--", linewidth=0.4)
...
Now let's estimate the optimal schedule and plot it against the current one: 
...
optimal_schedules = np.empty(
  (len(SIGMAS), len(DISTRIBUTIONS), len(betas))
)
...
for i, sigma in enumerate(SIGMAS):
  ...
  for j, (dist_name, dist_factory) in enumerate(DISTRIBUTIONS.items()):
    ...
    optimal_estimate = annealing_optimal(
      n_chains=len(betas),
      previous_schedule=betas,
      rejection_rates=reject_rates,
    )

    optimal_schedules[i, j] = optimal_estimate
...
We have the estimates of the optimal schedules. Let's check if they improve sampling:
...
for d, (_, dist_factory) in enumerate(DISTRIBUTIONS.items()):
  ...
  for s, sigma in enumerate(SIGMAS):
    betas_new = optimal_schedules[s, d]
    
    sigmas = sigma * jnp.ones_like(betas_new)
    sigmas = sigmas.at[0].set(_REFERENCE_SCALE)  # We know how to sample from the reference distribution 

    K_ind = generate_independent_annealed_kernel(
      log_prob=log_prob,
      log_ref=log_ref,
      annealing_schedule=betas_new,
      kernel_generator=generate_kernel,
      params=sigmas,
    )
