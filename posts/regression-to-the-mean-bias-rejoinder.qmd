...
- For $f=0.5$, $\hat Y_{0.5}$ is the regression to the mean we have seen above.
...
$$
which however has the form $(\hat Y_2 \mid X=x) = \mu + 2(x-\mu)$, which amplifies the distance from the mean!
...
More precisely, let's take a look at two conditional losses, measuring how wrong our predictions will be on average for a given value of $X$:
$$\begin{align*}
